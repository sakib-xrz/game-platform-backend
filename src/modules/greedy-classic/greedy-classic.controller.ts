...
import GreedyClassicService from './greedy-classic.services';
...
const getSnapshot = catchAsync(async (req: Request, res: Response) => {
  const data = await GreedyClassicService.getSnapshot(requireUserId(req));
  sendResponse(res, {
...
const placeBet = catchAsync(async (req: Request, res: Response) => {
  const data = await GreedyClassicService.placeBet(
    requireUserId(req),
...
const getMyBets = catchAsync(async (req: Request, res: Response) => {
  const result = await GreedyClassicService.getMyBets(
    requireUserId(req),
...
const getRoundHistory = catchAsync(async (req: Request, res: Response) => {
  const result = await GreedyClassicService.getRoundHistory(
    Number(req.query.page || 1),
    Number(req.query.limit || 20),
  );
  sendResponse(res, {
...
const getRound = catchAsync(async (req: Request, res: Response) => {
  const data = await GreedyClassicService.getRound(String(req.params.round_id));
  sendResponse(res, {
    statusCode: httpStatus.OK,
    success: true,
