import { z } from 'zod';
import { AdminRole, GreedyRoundStatus } from '@/generated/prisma/client';
import { createGreedyConfigSchema } from './game-admin.validation';
...
export const roundListSchema = z.object({
  query: paginationSchema.extend({
    status: z.nativeEnum(GreedyRoundStatus).optional(),
...
export const configBodySchema = createGreedyConfigSchema;
export const configUpdateSchema = z.object({
  params: z.object({ config_id: z.string().trim().cuid() }),
  body: createGreedyConfigSchema.shape.body,
});
