...
const getConfig = async (config_id: string) => {
  ...
  const config = await prisma.lucky77ConfigVersion.findFirst({
...
const updateDraft = async (config_id: string, payload: CreateLucky77ConfigBody, context: AdminAuditContext = {}) =>
  prisma.$transaction(async (tx) => {
    const game = await getGameOrThrow(tx);
    const options = await resolveLucky77OptionAssets(tx, payload.options);
    const target = await tx.lucky77ConfigVersion.findFirst({
      where: { id: config_id, game_id: game.id },
    ...
    await tx.lucky77ChipValueVersion.deleteMany({ where: { config_version_id: target.id } });
    await tx.lucky77OptionVersion.deleteMany({ where: { config_version_id: target.id } });
    const updated = await tx.lucky77ConfigVersion.update({
      where: { id: target.id },
      data: {
    ...
    await writeAdminAudit(tx, { ...context, outcome: 'success' }, { action: 'lucky_77.config.updated', entity_type: 'lucky_77_config_version', entity_id: target.id, old_values: toConfigJson(target), new_values: toConfigJson(updated) });
    return updated;
  });
...
const cloneConfig = async (config_id: string, context: AdminAuditContext = {}) =>
  prisma.$transaction(async (tx) => {
    const game = await getGameOrThrow(tx);
    const source = await tx.lucky77ConfigVersion.findFirst({ where: { id: config_id, game_id: game.id }, include: { options: true, chip_values: true } });
    if (!source) throw new AppError(httpStatus.NOT_FOUND, 'Config version not found');
    const latest = await tx.lucky77ConfigVersion.findFirst({ where: { game_id: game.id }, orderBy: { version: 'desc' }, select: { version: true } });
    const draft = await tx.lucky77ConfigVersion.create({
      data: {
    ...
    await writeAdminAudit(tx, { ...context, outcome: 'success' }, { action: 'lucky_77.config.cloned', entity_type: 'lucky_77_config_version', entity_id: draft.id, new_values: { source_config_id: source.id, source_version: source.version, version: draft.version } });
    return draft;
  });
  ...
  const [items, total] = await prisma.$transaction([
    prisma.lucky77Round.findMany({
      ...
      select: {
        ...
        result: { select: { id: true, winning_slot_index: true, algorithm_version: true, entropy_digest: true, audit_hash: true, generated_at: true, revealed_at: true, winning_option: { select: { id: true, code: true, name: true, image_url: true } } } },
...
const getRound = async (round_id: string, role?: AdminRole) => {
  const game = await getGameOrThrow();
  const round = await prisma.lucky77Round.findFirst({
  ...
  const [betTotals, payoutTotals, refundTotals, outcomes] = await Promise.all([
    prisma.lucky77Bet.aggregate({ where: { round_id }, _sum: { amount: true }, _count: { _all: true } }),
    prisma.lucky77UserPayout.aggregate({ where: { round_id }, _sum: { total_payout: true, total_winning_stake: true }, _count: { _all: true } }),
    prisma.lucky77UserRefund.aggregate({ where: { round_id }, _sum: { total_bet_amount: true }, _count: { _all: true } }),
    prisma.lucky77BetSettlement.groupBy({ by: ['outcome'], where: { round_id }, _count: { _all: true }, _sum: { payout_amount: true } }),
  ]);
...
const verifyRoundResult = async (round_id: string, role?: AdminRole) => {
  const game = await getGameOrThrow();
  const round = await prisma.lucky77Round.findFirst({ where: { id: round_id, game_id: game.id }, include: { result: { include: { winning_option: { select: { id: true, code: true, name: true } } } } } });
  if (!round) throw new AppError(httpStatus.NOT_FOUND, 'Round not found');
  if (!round.result) throw new AppError(httpStatus.CONFLICT, 'Round has no immutable result to verify');
  const result = round.result;
  const expected_hash = sha256([round.id, result.config_version_id, result.winning_option_version_id, String(result.winning_slot_index), result.algorithm_version, result.entropy_digest, result.generated_at.toISOString()].join('|'));
  const privileged = canViewEntropy(role);
  return { verified: expected_hash === result.audit_hash, round_id: round.id, result_id: result.id, algorithm_version: result.algorithm_version, winning_slot_index: result.winning_slot_index, generated_at: result.generated_at, revealed_at: result.revealed_at, winning_option: result.winning_option, audit_hash: privileged ? result.audit_hash : undefined, expected_hash: privileged ? expected_hash : undefined };
};
...
const getUserSummary = async (user_id: string) => {
  ...
  const [wallet, betsCount, betTotal, payoutTotal, refundTotal] = await Promise.all([
    ...
    prisma.lucky77Bet.aggregate({ where: { user_id }, _sum: { amount: true } }),
    prisma.lucky77UserPayout.aggregate({ where: { user_id }, _sum: { total_payout: true } }),
    prisma.lucky77UserRefund.aggregate({ where: { user_id }, _sum: { total_bet_amount: true } }),
  ]);
  ...
  const bets = await prisma.lucky77Bet.findMany({ where: { user_id }, orderBy: { created_at: 'desc' }, take: 100, select: { id: true, round_id: true, amount: true, accepted_at: true, option: { select: { code: true, name: true, image_url: true } }, settlement: { select: { outcome: true, payout_amount: true, settled_at: true } } } });
...
const getMetrics = async (query: OpsMetricsQuery) => {
  ...
  const [rounds, bets, payouts, refunds, ledger, uniqueBettors, betSeries, refundSeries, payoutSeries, roundSeries, settlementLatency] = await Promise.all([
    prisma.lucky77Round.groupBy({ by: ['status'], where: { game_id: game.id, created_at: { gte: from, lte: to } }, _count: { _all: true } }),
    prisma.lucky77Bet.aggregate({ where: { game_id: game.id, created_at: { gte: from, lte: to } }, _count: { _all: true }, _sum: { amount: true } }),
    prisma.lucky77UserPayout.aggregate({ where: { round: { game_id: game.id }, created_at: { gte: from, lte: to } }, _count: { _all: true }, _sum: { total_payout: true, total_winning_stake: true } }),
    prisma.lucky77UserRefund.aggregate({ where: { round: { game_id: game.id }, created_at: { gte: from, lte: to } }, _count: { _all: true }, _sum: { total_bet_amount: true } }),
    prisma.walletLedger.groupBy({ by: ['type'], where: { game_id: game.id, created_at: { gte: from, lte: to } }, _count: { _all: true }, _sum: { amount: true } }),
    prisma.lucky77Bet.findMany({ where: { game_id: game.id, created_at: { gte: from, lte: to } }, distinct: ['user_id'], select: { user_id: true } }),
    prisma.$queryRaw<Array<{ bucket: string; amount: string; count: string; unique_bettors: string }>>(Prisma.sql`SELECT to_char((created_at AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Dhaka', 'YYYY-MM-DD') AS bucket, COALESCE(SUM(amount), 0)::text AS amount, COUNT(*)::text AS count, COUNT(DISTINCT user_id)::text AS unique_bettors FROM lucky_77_bets WHERE game_id = ${game.id} AND created_at >= ${from} AND created_at <= ${to} GROUP BY bucket ORDER BY bucket`),
    prisma.$queryRaw<Array<{ bucket: string; amount: string }>>(Prisma.sql`SELECT to_char((r.created_at AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Dhaka', 'YYYY-MM-DD') AS bucket, COALESCE(SUM(f.total_bet_amount), 0)::text AS amount FROM lucky_77_user_refunds f JOIN lucky_77_rounds r ON r.id = f.round_id WHERE r.game_id = ${game.id} AND f.created_at >= ${from} AND f.created_at <= ${to} GROUP BY bucket ORDER BY bucket`),
    prisma.$queryRaw<Array<{ bucket: string; amount: string }>>(Prisma.sql`SELECT to_char((r.created_at AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Dhaka', 'YYYY-MM-DD') AS bucket, COALESCE(SUM(p.total_payout), 0)::text AS amount FROM lucky_77_user_payouts p JOIN lucky_77_rounds r ON r.id = p.round_id WHERE r.game_id = ${game.id} AND p.created_at >= ${from} AND p.created_at <= ${to} GROUP BY bucket ORDER BY bucket`),
...
const getOperationsHealth = async () => {
  ...
  const [game, lease, outbox] = await Promise.all([
    prisma.game.findUnique({ where: { code: LUCKY_77_GAME_CODE }, include: { lucky_77_runtime_state: true } }),
    prisma.workerLease.findUnique({ where: { lease_key: WORKER_LEASE_KEY } }),
    prisma.outboxEvent.groupBy({ by: ['status'], _count: { _all: true } }),
  ]);
  const now = new Date();
  return { database, redis: redisClient.isReady ? 'ready' : 'not_ready', game: game ? { status: game.status, runtime_status: game.lucky_77_runtime_state?.status ?? null, revision: game.lucky_77_runtime_state?.revision ?? null } : null, worker: lease ? { owner_id: lease.owner_id, lease_until: lease.lease_until, heartbeat_at: lease.heartbeat_at, healthy: lease.lease_until > now } : { healthy: false }, outbox: Object.fromEntries(outbox.map((item) => [item.status, item._count._all])) };
};
...
export const refreshOperationalAlerts = async () => {
  ...
  if (health.database !== 'ready') {
    if (Date.now() - lastDatabaseUnavailableWebhookAt >= 5 * 60_000) {
      ...
      await deliverOpsWebhook({ event_id: `database-unavailable-${lastDatabaseUnavailableWebhookAt}`, source: 'lucky-77-ops', code: 'database_unavailable', severity: 'critical', message: 'PostgreSQL is unavailable', metadata: {}, occurred_at: new Date().toISOString() }).catch((error) => logger.error('ops_alert_webhook_failed', { code: 'database_unavailable', error }));
    }
  ...
  if (!health.worker.healthy && health.game?.status === 'active') conditions.push({ code: 'lucky_77_worker_unhealthy', severity: 'critical', message: 'Lucky 77 worker lease is absent or expired', metadata: { worker: health.worker } });
  if (health.game?.runtime_status === 'degraded') conditions.push({ code: 'lucky_77_runtime_degraded', severity: 'critical', message: 'Lucky 77 runtime is degraded', metadata: {} });
  ...
  const [stalled_settlement, stalled_refund] = await Promise.all([
    prisma.lucky77Round.findFirst({ where: { game: { code: LUCKY_77_GAME_CODE }, status: Lucky77RoundStatus.settling, settlement_started_at: { lt: stalled_before } }, orderBy: { settlement_started_at: 'asc' }, select: { id: true, round_number: true, settlement_started_at: true } }),
    prisma.lucky77Round.findFirst({ where: { game: { code: LUCKY_77_GAME_CODE }, status: Lucky77RoundStatus.cancelled, cancelled_at: { lt: stalled_before } }, orderBy: { cancelled_at: 'asc' }, select: { id: true, round_number: true, cancelled_at: true } }),
  ]);
  if (stalled_settlement) conditions.push({ code: 'settlement_failed_or_stalled', severity: 'critical', message: 'A Lucky 77 round settlement is stalled', metadata: { round_id: stalled_settlement.id, round_number: stalled_settlement.round_number.toString(), settlement_started_at: stalled_settlement.settlement_started_at?.toISOString() } });
  if (stalled_refund) conditions.push({ code: 'refund_failed_or_stalled', severity: 'critical', message: 'A cancelled Lucky 77 round refund is stalled', metadata: { round_id: stalled_refund.id, round_number: stalled_refund.round_number.toString(), cancelled_at: stalled_refund.cancelled_at?.toISOString() } });
  const seen = new Set(conditions.map((item) => item.code));
  for (const condition of conditions) {
    const dedupe_key = `lucky-77:${condition.code}`;
    ...
    const alert = existing
      ...
      : await prisma.opsAlert.create({ data: { code: condition.code, severity: condition.severity, status: OpsAlertStatus.open, message: condition.message, source: 'lucky-77-ops', dedupe_key, metadata: condition.metadata as Prisma.InputJsonObject } });
    if (should_notify && condition.severity === 'critical') {
      await deliverOpsWebhook({ event_id: alert.id, source: 'lucky-77-ops', code: condition.code, severity: condition.severity, message: condition.message, metadata: condition.metadata, occurred_at: alert.first_seen_at.toISOString() }).catch((error) => logger.error('ops_alert_webhook_failed', { alert_id: alert.id, code: condition.code, error }));
    }
  }
  const stale = await prisma.opsAlert.findMany({ where: { status: { in: [OpsAlertStatus.open, OpsAlertStatus.acknowledged] }, dedupe_key: { startsWith: 'lucky-77:' }, last_seen_at: { lt: new Date(Date.now() - 90_000) } }, select: { id: true } });
...
const getOverview = async () => {
  const [health, metrics, runtime, recent] = await Promise.all([
    ...
    prisma.lucky77RuntimeState.findFirst({ where: { game: { code: LUCKY_77_GAME_CODE } }, include: { current_round: true, active_config_version: { select: { id: true, version: true, status: true } } } }),
    prisma.lucky77Round.findMany({ where: { game: { code: LUCKY_77_GAME_CODE } }, orderBy: { round_number: 'desc' }, take: 10, select: { id: true, round_number: true, status: true, result: { select: { winning_slot_index: true, winning_option: { select: { code: true, name: true, image_url: true } } } }, _count: { select: { bets: true } } } }),
  ]);
...
const setAvailability = async (status: GameStatus, context: AdminAuditContext = {}) =>
  prisma.$transaction(async (tx) => {
    ...
    if (status === GameStatus.active && !runtime.active_config_version_id) throw new AppError(httpStatus.CONFLICT, 'Publish a Lucky 77 config before activating the game');
    const nextRuntime = status === GameStatus.active
      ? Lucky77RuntimeStatus.running
      : status === GameStatus.disabled
        ? Lucky77RuntimeStatus.stopped
        : Lucky77RuntimeStatus.paused;
    await tx.game.update({ where: { id: game.id }, data: { status } });
    const updated = await tx.lucky77RuntimeState.update({ where: { game_id: game.id }, data: { status: nextRuntime, revision: { increment: 1 } } });
    await writeAdminAudit(tx, { ...context, outcome: 'success' }, { action: 'lucky_77.game.availability_changed', entity_type: 'game', entity_id: game.id, old_values: { status: game.status }, new_values: { status } });
    await tx.outboxEvent.create({ data: { aggregate_type: 'game', aggregate_id: game.id, event_type: 'platform.game.availability_changed', socket_room: LUCKY_77_SOCKET_ROOM, payload: { game_code: LUCKY_77_GAME_CODE, status } } });
