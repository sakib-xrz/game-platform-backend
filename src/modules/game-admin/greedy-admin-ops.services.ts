...
const updateDraft = async (config_id: string, payload: CreateGreedyConfigBody, context: AdminAuditContext = {}) =>
  prisma.$transaction(async (tx) => {
    ...
    const [referencedRound, referencedResult] = await Promise.all([
      tx.greedyRound.findFirst({ where: { config_version_id: target.id }, select: { id: true } }),
      tx.greedyRoundResult.findFirst({ where: { config_version_id: target.id }, select: { id: true } }),
    ]);
...
const listRoundBets = async (round_id: string, query: OpsRoundBetsQuery) => {
  ...
  const where: Prisma.GreedyBetWhereInput = {
    round_id,
