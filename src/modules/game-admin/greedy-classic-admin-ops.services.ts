...
const updateDraft = async (config_id: string, payload: CreateGreedyClassicConfigBody, context: AdminAuditContext = {}) =>
  prisma.$transaction(async (tx) => {
    ...
    const updated = await tx.greedyClassicConfigVersion.update({
      where: { id: target.id },
      data: {
...
const listRoundBets = async (round_id: string, query: OpsRoundBetsQuery) => {
  ...
  if (!round) throw new AppError(httpStatus.NOT_FOUND, 'Round not found');
  const pagination = getPagination(query.page, query.limit);
  const where: Prisma.GreedyClassicBetWhereInput = { round_id, ...(query.user_id ? { user_id: query.user_id } : {}), ...(query.option_id ? { option_version_id: query.option_id } : {}) };
  const [items, total] = await prisma.$transaction([
