...
import type { CreateGreedyClassicConfigBody } from './game-admin.validation';
import { resolveGreedyClassicOptionAssets } from './greedy-classic-admin.services';
...
const WORKER_LEASE_KEY = 'game-worker:greedy-classic';
...
const getGameOrThrow = async (tx: Prisma.TransactionClient | typeof prisma = prisma) => {
  ...
  if (!game) throw new AppError(httpStatus.NOT_FOUND, 'Greedy Classic game not initialized');
  return game;
};
...
const getConfig = async (config_id: string) => {
  ...
  const config = await prisma.greedyClassicConfigVersion.findFirst({
...
const updateDraft = async (config_id: string, payload: CreateGreedyClassicConfigBody, context: AdminAuditContext = {}) =>
  prisma.$transaction(async (tx) => {
    ...
    if (hasRoundRefs) {
      ...
      for (const item of options) {
        ...
        await tx.greedyClassicOptionVersion.update({
      ...
      await tx.greedyClassicChipValueVersion.deleteMany({ where: { config_version_id: target.id } });
      if (payload.chip_values.length) {
        await tx.greedyClassicChipValueVersion.createMany({
      ...
      updated = await tx.greedyClassicConfigVersion.update({
        where: { id: target.id },
        data: rootData,
      ...
      await tx.greedyClassicChipValueVersion.deleteMany({ where: { config_version_id: target.id } });
      await tx.greedyClassicOptionVersion.deleteMany({ where: { config_version_id: target.id } });
      updated = await tx.greedyClassicConfigVersion.update({
        where: { id: target.id },
        data: {
          ...rootData,
    ...
    if (target.status === ConfigVersionStatus.published) {
      ...
      if (runtime?.active_config_version_id === target.id) {
        await tx.greedyClassicRuntimeState.update({ where: { game_id: game.id }, data: { revision: { increment: 1 } } });
        await tx.outboxEvent.create({
          data: {
            aggregate_type: 'greedy_classic_config_version',
            aggregate_id: target.id,
            event_type: 'greedy_classic.config.updated',
...
const cloneConfig = async (config_id: string, context: AdminAuditContext = {}) =>
  prisma.$transaction(async (tx) => {
    const game = await getGameOrThrow(tx);
    const source = await tx.greedyClassicConfigVersion.findFirst({ where: { id: config_id, game_id: game.id }, include: { options: true, chip_values: true } });
    if (!source) throw new AppError(httpStatus.NOT_FOUND, 'Config version not found');
    const latest = await tx.greedyClassicConfigVersion.findFirst({ where: { game_id: game.id }, orderBy: { version: 'desc' }, select: { version: true } });
    const draft = await tx.greedyClassicConfigVersion.create({
      data: {
    ...
    await writeAdminAudit(tx, { ...context, outcome: 'success' }, { action: 'greedy_classic.config.cloned', entity_type: 'greedy_classic_config_version', entity_id: draft.id, new_values: { source_config_id: source.id, source_version: source.version, version: draft.version } });
    return draft;
  });
...
const getRound = async (round_id: string, role?: AdminRole) => {
  const game = await getGameOrThrow();
  const round = await prisma.greedyClassicRound.findFirst({
  ...
  const [betTotals, payoutTotals, refundTotals, outcomes] = await Promise.all([
    prisma.greedyClassicBet.aggregate({ where: { round_id }, _sum: { amount: true }, _count: { _all: true } }),
    prisma.greedyClassicUserPayout.aggregate({ where: { round_id }, _sum: { total_payout: true, total_winning_stake: true }, _count: { _all: true } }),
    prisma.greedyClassicUserRefund.aggregate({ where: { round_id }, _sum: { total_bet_amount: true }, _count: { _all: true } }),
    prisma.greedyClassicBetSettlement.groupBy({ by: ['outcome'], where: { round_id }, _count: { _all: true }, _sum: { payout_amount: true } }),
  ]);
...
const verifyRoundResult = async (round_id: string, role?: AdminRole) => {
  const game = await getGameOrThrow();
  const round = await prisma.greedyClassicRound.findFirst({ where: { id: round_id, game_id: game.id }, include: { result: { include: { winning_option: { select: { id: true, code: true, name: true } } } } } });
  if (!round) throw new AppError(httpStatus.NOT_FOUND, 'Round not found');
...
const getUserSummary = async (user_id: string) => {
  ...
  const [wallet, betsCount, betTotal, payoutTotal, refundTotal] = await Promise.all([
    ...
    prisma.greedyClassicBet.aggregate({ where: { user_id }, _sum: { amount: true } }),
    prisma.greedyClassicUserPayout.aggregate({ where: { user_id }, _sum: { total_payout: true } }),
    prisma.greedyClassicUserRefund.aggregate({ where: { user_id }, _sum: { total_bet_amount: true } }),
  ]);
  ...
  const bets = await prisma.greedyClassicBet.findMany({ where: { user_id }, orderBy: { created_at: 'desc' }, take: 100, select: { id: true, round_id: true, amount: true, accepted_at: true, option: { select: { code: true, name: true, image_url: true } }, settlement: { select: { outcome: true, payout_amount: true, settled_at: true } } } });
...
const getMetrics = async (query: OpsMetricsQuery) => {
  ...
  const [rounds, bets, payouts, refunds, ledger, uniqueBettors, betSeries, refundSeries, payoutSeries, roundSeries, settlementLatency] = await Promise.all([
    prisma.greedyClassicRound.groupBy({ by: ['status'], where: { game_id: game.id, created_at: { gte: from, lte: to } }, _count: { _all: true } }),
    prisma.greedyClassicBet.aggregate({ where: { game_id: game.id, created_at: { gte: from, lte: to } }, _count: { _all: true }, _sum: { amount: true } }),
    prisma.greedyClassicUserPayout.aggregate({ where: { round: { game_id: game.id }, created_at: { gte: from, lte: to } }, _count: { _all: true }, _sum: { total_payout: true, total_winning_stake: true } }),
    prisma.greedyClassicUserRefund.aggregate({ where: { round: { game_id: game.id }, created_at: { gte: from, lte: to } }, _count: { _all: true }, _sum: { total_bet_amount: true } }),
    prisma.walletLedger.groupBy({ by: ['type'], where: { game_id: game.id, created_at: { gte: from, lte: to } }, _count: { _all: true }, _sum: { amount: true } }),
    prisma.greedyClassicBet.findMany({ where: { game_id: game.id, created_at: { gte: from, lte: to } }, distinct: ['user_id'], select: { user_id: true } }),
    prisma.$queryRaw<Array<{ bucket: string; amount: string; count: string; unique_bettors: string }>>(Prisma.sql`SELECT to_char((created_at AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Dhaka', 'YYYY-MM-DD') AS bucket, COALESCE(SUM(amount), 0)::text AS amount, COUNT(*)::text AS count, COUNT(DISTINCT user_id)::text AS unique_bettors FROM greedy_classic_bets WHERE game_id = ${game.id} AND created_at >= ${from} AND created_at <= ${to} GROUP BY bucket ORDER BY bucket`),
    prisma.$queryRaw<Array<{ bucket: string; amount: string }>>(Prisma.sql`SELECT to_char((r.created_at AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Dhaka', 'YYYY-MM-DD') AS bucket, COALESCE(SUM(f.total_bet_amount), 0)::text AS amount FROM greedy_classic_user_refunds f JOIN greedy_classic_rounds r ON r.id = f.round_id WHERE r.game_id = ${game.id} AND f.created_at >= ${from} AND f.created_at <= ${to} GROUP BY bucket ORDER BY bucket`),
    prisma.$queryRaw<Array<{ bucket: string; amount: string }>>(Prisma.sql`SELECT to_char((r.created_at AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Dhaka', 'YYYY-MM-DD') AS bucket, COALESCE(SUM(p.total_payout), 0)::text AS amount FROM greedy_classic_user_payouts p JOIN greedy_classic_rounds r ON r.id = p.round_id WHERE r.game_id = ${game.id} AND p.created_at >= ${from} AND p.created_at <= ${to} GROUP BY bucket ORDER BY bucket`),
    prisma.$queryRaw<Array<{ bucket: string; rounds: string; cancelled: string }>>(Prisma.sql`SELECT to_char((created_at AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Dhaka', 'YYYY-MM-DD') AS bucket, COUNT(*)::text AS rounds, COUNT(*) FILTER (WHERE status = 'cancelled')::text AS cancelled FROM greedy_classic_rounds WHERE game_id = ${game.id} AND created_at >= ${from} AND created_at <= ${to} GROUP BY bucket ORDER BY bucket`),
    prisma.$queryRaw<Array<{ average_seconds: number | null; settled_rounds: string }>>(Prisma.sql`SELECT AVG(EXTRACT(EPOCH FROM (settled_at - result_reveal_at)))::float8 AS average_seconds, COUNT(*)::text AS settled_rounds FROM greedy_classic_rounds WHERE game_id = ${game.id} AND settled_at IS NOT NULL AND result_reveal_at IS NOT NULL AND settled_at >= ${from} AND settled_at <= ${to}`),
  ]);
  ...
  return {
    ...
    rounds: { by_status: rounds, total: rounds.reduce((sum, row) => sum + row._count._all, 0), cancelled: rounds.find((row) => row.status === GreedyClassicRoundStatus.cancelled)?._count._all ?? 0, cancellation_rate: rounds.length ? ((rounds.find((row) => row.status === GreedyClassicRoundStatus.cancelled)?._count._all ?? 0) / rounds.reduce((sum, row) => sum + row._count._all, 0)) * 100 : 0 },
...
const getOperationsHealth = async () => {
  ...
  const [game, lease, outbox] = await Promise.all([
    prisma.game.findUnique({ where: { code: GREEDY_CLASSIC_GAME_CODE }, include: { greedy_classic_runtime_state: true } }),
    prisma.workerLease.findUnique({ where: { lease_key: WORKER_LEASE_KEY } }),
    prisma.outboxEvent.groupBy({ by: ['status'], _count: { _all: true } }),
  ]);
  const now = new Date();
  return { database, redis: redisClient.isReady ? 'ready' : 'not_ready', game: game ? { status: game.status, runtime_status: game.greedy_classic_runtime_state?.status ?? null, revision: game.greedy_classic_runtime_state?.revision ?? null } : null, worker: lease ? { owner_id: lease.owner_id, lease_until: lease.lease_until, heartbeat_at: lease.heartbeat_at, healthy: lease.lease_until > now } : { healthy: false }, outbox: Object.fromEntries(outbox.map((item) => [item.status, item._count._all])) };
};
...
export const refreshOperationalAlerts = async () => {
  ...
  if (health.database !== 'ready') {
    if (Date.now() - lastDatabaseUnavailableWebhookAt >= 5 * 60_000) {
      ...
      await deliverOpsWebhook({ event_id: `database-unavailable-${lastDatabaseUnavailableWebhookAt}`, source: 'greedy-classic-ops', code: 'database_unavailable', severity: 'critical', message: 'PostgreSQL is unavailable', metadata: {}, occurred_at: new Date().toISOString() }).catch((error) => logger.error('ops_alert_webhook_failed', { code: 'database_unavailable', error }));
    }
  ...
  if (!health.worker.healthy && health.game?.status === 'active') conditions.push({ code: 'greedy_classic_worker_unhealthy', severity: 'critical', message: 'Greedy Classic worker lease is absent or expired', metadata: { worker: health.worker } });
  if (health.game?.runtime_status === 'degraded') conditions.push({ code: 'greedy_classic_runtime_degraded', severity: 'critical', message: 'Greedy Classic runtime is degraded', metadata: {} });
  ...
  const [stalled_settlement, stalled_refund] = await Promise.all([
    prisma.greedyClassicRound.findFirst({ where: { game: { code: GREEDY_CLASSIC_GAME_CODE }, status: GreedyClassicRoundStatus.settling, settlement_started_at: { lt: stalled_before } }, orderBy: { settlement_started_at: 'asc' }, select: { id: true, round_number: true, settlement_started_at: true } }),
    prisma.greedyClassicRound.findFirst({ where: { game: { code: GREEDY_CLASSIC_GAME_CODE }, status: GreedyClassicRoundStatus.cancelled, cancelled_at: { lt: stalled_before } }, orderBy: { cancelled_at: 'asc' }, select: { id: true, round_number: true, cancelled_at: true } }),
  ]);
  ...
  for (const condition of conditions) {
    const dedupe_key = `greedy-classic:${condition.code}`;
    ...
    const alert = existing
      ...
      : await prisma.opsAlert.create({ data: { code: condition.code, severity: condition.severity, status: OpsAlertStatus.open, message: condition.message, source: 'greedy-classic-ops', dedupe_key, metadata: condition.metadata as Prisma.InputJsonObject } });
    if (should_notify && condition.severity === 'critical') {
      await deliverOpsWebhook({ event_id: alert.id, source: 'greedy-classic-ops', code: condition.code, severity: condition.severity, message: condition.message, metadata: condition.metadata, occurred_at: alert.first_seen_at.toISOString() }).catch((error) => logger.error('ops_alert_webhook_failed', { alert_id: alert.id, code: condition.code, error }));
    }
  }
  const stale = await prisma.opsAlert.findMany({ where: { status: { in: [OpsAlertStatus.open, OpsAlertStatus.acknowledged] }, dedupe_key: { startsWith: 'greedy-classic:' }, last_seen_at: { lt: new Date(Date.now() - 90_000) } }, select: { id: true } });
...
const getOverview = async () => {
  const [health, metrics, runtime, recent] = await Promise.all([
    ...
    prisma.greedyClassicRuntimeState.findFirst({ where: { game: { code: GREEDY_CLASSIC_GAME_CODE } }, include: { current_round: true, active_config_version: { select: { id: true, version: true, status: true } } } }),
    prisma.greedyClassicRound.findMany({ where: { game: { code: GREEDY_CLASSIC_GAME_CODE } }, orderBy: { round_number: 'desc' }, take: 10, select: { id: true, round_number: true, status: true, result: { select: { winning_option: { select: { code: true, name: true, image_url: true } } } }, _count: { select: { bets: true } } } }),
  ]);
...
const setAvailability = async (status: GameStatus, context: AdminAuditContext = {}) =>
  prisma.$transaction(async (tx) => {
    const game = await getGameOrThrow(tx);
    const runtime = await tx.greedyClassicRuntimeState.findUnique({ where: { game_id: game.id } });
    if (!runtime) throw new AppError(httpStatus.CONFLICT, 'Greedy Classic runtime is not initialized');
    ...
    if (status === GameStatus.active && !runtime.active_config_version_id) throw new AppError(httpStatus.CONFLICT, 'Publish a Greedy Classic config before activating the game');
    const nextRuntime = status === GameStatus.active
      ? GreedyClassicRuntimeStatus.running
      : status === GameStatus.disabled
        ? GreedyClassicRuntimeStatus.stopped
        : GreedyClassicRuntimeStatus.paused;
    await tx.game.update({ where: { id: game.id }, data: { status } });
    const updated = await tx.greedyClassicRuntimeState.update({ where: { game_id: game.id }, data: { status: nextRuntime, revision: { increment: 1 } } });
    await writeAdminAudit(tx, { ...context, outcome: 'success' }, { action: 'greedy_classic.game.availability_changed', entity_type: 'game', entity_id: game.id, old_values: { status: game.status }, new_values: { status } });
    await tx.outboxEvent.create({ data: { aggregate_type: 'game', aggregate_id: game.id, event_type: 'platform.game.availability_changed', socket_room: GREEDY_CLASSIC_SOCKET_ROOM, payload: { game_code: GREEDY_CLASSIC_GAME_CODE, status } } });
