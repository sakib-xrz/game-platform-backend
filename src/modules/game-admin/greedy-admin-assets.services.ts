...
const quarantineKey = (): string => `greedy/quarantine/${randomUUID()}`;
const publishedKey = (checksum: string): string => `greedy/options/sha256/${checksum}.webp`;
...
const auditReuse = async (context: AdminAuditContext, asset_id: string, source: string): Promise<void> =>
  prisma.$transaction((tx) => writeAdminAudit(tx, { ...context, outcome: 'success' }, { action: 'greedy.asset.reused', entity_type: 'admin_asset', entity_id: asset_id, new_values: { source } }));
...
const uploadAsset = async (file: Express.Multer.File | undefined, context: AdminAuditContext = {}) => {
  ...
  try {
    return await prisma.$transaction(async (tx) => {
      ...
      await writeAdminAudit(tx, { ...context, outcome: 'success' }, { action: 'greedy.asset.uploaded', entity_type: 'admin_asset', entity_id: asset.id, new_values: { object_key: key, checksum_sha256: normalized.checksum, content_type: 'image/webp', byte_size: normalized.buffer.byteLength } });
      return asset;
    });
  } catch (error) {
...
const presignAsset = async (input: { content_type: AllowedContentType; byte_size: number; checksum_sha256: string }, context: AdminAuditContext = {}) => {
  ...
  await prisma.$transaction((tx) => writeAdminAudit(tx, { ...context, outcome: 'success' }, { action: 'greedy.asset.presigned', entity_type: 'admin_asset', entity_id: asset.id, new_values: { content_type: input.content_type, byte_size: input.byte_size, checksum_sha256: checksum } }));
  ...
  try {
    ...
    if (existing) {
      ...
      await prisma.$transaction(async (tx) => {
        ...
        await writeAdminAudit(tx, { ...context, outcome: 'success' }, { action: 'greedy.asset.reused', entity_type: 'admin_asset', entity_id: existing.id, new_values: { source: 'presigned_completion', duplicate_asset_id: asset.id } });
      });
      return existing;
    }
    const key = publishedKey(normalized.checksum);
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      await writeAdminAudit(tx, { ...context, outcome: 'success' }, { action: 'greedy.asset.completed', entity_type: 'admin_asset', entity_id: asset.id, new_values: { object_key: key, checksum_sha256: normalized.checksum, byte_size: normalized.buffer.byteLength } });
      return updated;
    });
  } catch (error) {
    ...
    await prisma.$transaction((tx) => writeAdminAudit(tx, { ...context, outcome: 'failed' }, { action: 'greedy.asset.completion_failed', entity_type: 'admin_asset', entity_id: asset.id, new_values: { reason: error instanceof Error ? error.message.slice(0, 240) : 'unknown_error' } })).catch(() => undefined);
    await deleteObject(asset.object_key).catch(() => undefined);
    throw error;
  }
};
...
const deleteAsset = async (asset_id: string, context: AdminAuditContext = {}) => {
  ...
  const deleted = await prisma.$transaction(async (tx) => {
    ...
    await writeAdminAudit(tx, { ...context, outcome: 'success' }, { action: 'greedy.asset.deleted', entity_type: 'admin_asset', entity_id: asset.id, old_values: { status: asset.status, object_key: asset.object_key }, new_values: { status: AdminAssetStatus.deleted } });
    return updated;
  });
  await deleteObject(asset.object_key);
  return deleted;
