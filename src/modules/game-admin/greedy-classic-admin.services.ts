...
export const resolveGreedyClassicOptionAssets = async (tx: Prisma.TransactionClient, options: CreateGreedyClassicConfigBody['options']): Promise<CreateGreedyClassicConfigBody['options']> => {
...
const createConfig = async (payload: CreateGreedyClassicConfigBody, context: AdminAuditContext = {}) =>
  prisma.$transaction(async (tx) => {
    const game = await getGameOrThrow(tx);
    const options = await resolveGreedyClassicOptionAssets(tx, payload.options);
    const latest = await tx.greedyClassicConfigVersion.findFirst({
    ...
    const config = await tx.greedyClassicConfigVersion.create({
      data: {
        game_id: game.id,
        version,
    ...
    await writeAdminAudit(tx, { ...context, outcome: 'success' }, { action: 'greedy_classic.config.created', entity_type: 'greedy_classic_config_version', entity_id: config.id, new_values: { version } });
...
const listConfigs = async () => {
  ...
  return prisma.greedyClassicConfigVersion.findMany({
    where: { game_id: game.id },
    include: {
      options: { orderBy: { display_order: 'asc' } },
      chip_values: { orderBy: { display_order: 'asc' } },
...
const validateConfig = async (payload: CreateGreedyClassicConfigBody) => {
...
const getRuntime = async () => {
  const game = await getGameOrThrow();
  return prisma.greedyClassicRuntimeState.findUnique({
    where: { game_id: game.id },
    include: {
      current_round: true,
...
const resume = async (context: AdminAuditContext = {}) =>
  prisma.$transaction(async (tx) => {
    ...
    if (!runtime?.active_config_version_id) {
      throw new AppError(httpStatus.CONFLICT, 'Publish a Greedy Classic config before starting the game');
    }
    await tx.game.update({ where: { id: game.id }, data: { status: GameStatus.active } });
    const updated = await tx.greedyClassicRuntimeState.update({
      where: { game_id: game.id },
      data: { status: GreedyClassicRuntimeStatus.running, revision: { increment: 1 } },
    });
    await writeAdminAudit(tx, { ...context, outcome: 'success' }, { action: 'greedy_classic.game.resumed', entity_type: 'game', entity_id: game.id });
    await tx.outboxEvent.create({
      data: {
        aggregate_type: 'game', aggregate_id: game.id,
        event_type: 'platform.game.resumed', socket_room: GREEDY_CLASSIC_SOCKET_ROOM,
        payload: { game_code: GREEDY_CLASSIC_GAME_CODE },
      },
    });
    return updated;
  });
...
const pause = async (context: AdminAuditContext = {}) =>
  prisma.$transaction(async (tx) => {
    ...
    const updated = await tx.greedyClassicRuntimeState.update({
      where: { game_id: game.id },
      data: { status: GreedyClassicRuntimeStatus.paused, revision: { increment: 1 } },
    });
    await writeAdminAudit(tx, { ...context, outcome: 'success' }, { action: 'greedy_classic.game.paused', entity_type: 'game', entity_id: game.id });
    await tx.outboxEvent.create({
      data: {
        aggregate_type: 'game', aggregate_id: game.id,
        event_type: 'platform.game.paused', socket_room: GREEDY_CLASSIC_SOCKET_ROOM,
        payload: { game_code: GREEDY_CLASSIC_GAME_CODE, current_round_will_finish: true },
      },
    });
    return updated;
  });
...
const cancelCurrentRound = async (payload: CancelRoundBody, context: AdminAuditContext = {}) =>
  prisma.$transaction(async (tx) => {
    ...
    await tx.$queryRaw(Prisma.sql`SELECT id FROM greedy_classic_rounds WHERE id = ${runtime.current_round_id} FOR UPDATE`);
    const round = await tx.greedyClassicRound.findUnique({ where: { id: runtime.current_round_id }, include: { config_version: { select: { lock_duration_ms: true, drawing_duration_ms: true, result_duration_ms: true } } } });
    const cancellable_statuses: GreedyClassicRoundStatus[] = [
      GreedyClassicRoundStatus.created,
      GreedyClassicRoundStatus.betting_open,
      GreedyClassicRoundStatus.betting_locked,
    ];
    ...
    const exposure = await tx.greedyClassicBet.aggregate({ where: { round_id: round.id }, _sum: { amount: true }, _count: { _all: true } });
    ...
    if (!payload.approval_id && exposure_amount >= threshold) {
      ...
      const approval = await createPendingApproval({ admin_user_id: context.admin_user_id, action_type: 'greedy_classic.round.cancel', target_type: 'greedy_classic_round', target_id: round.id, payload: { round_id: round.id, reason: payload.reason, exposure: exposure_amount.toString(), current_round_id: runtime.current_round_id }, idempotency_key: context.idempotency_key, expires_at }, tx);
      await writeAdminAudit(tx, { ...context, approval_request_id: approval.id, outcome: 'success' }, { action: 'greedy_classic.round.cancellation_submitted_for_approval', entity_type: 'greedy_classic_round', entity_id: round.id, new_values: { reason: payload.reason, exposure: exposure_amount.toString(), bet_count: exposure._count._all } });
    ...
    if (payload.approval_id) {
      ...
      if (!approval || approval.action_type !== 'greedy_classic.round.cancel' || approval.status !== AdminApprovalStatus.approved || approval.expires_at <= new Date()) throw new AppError(httpStatus.CONFLICT, 'Round cancellation approval is not ready');
    ...
    const cancellation = await tx.greedyClassicRound.updateMany({
      where: { id: round.id, status: { in: cancellable_statuses } },
      data: {
        status: GreedyClassicRoundStatus.cancelled,
    ...
    const cancelled = await tx.greedyClassicRound.findUniqueOrThrow({ where: { id: round.id } });
    await writeAdminAudit(tx, { ...context, approval_request_id: payload.approval_id, outcome: 'success' }, { action: 'greedy_classic.round.cancelled', entity_type: 'greedy_classic_round', entity_id: round.id, new_values: { reason: payload.reason } });
    ...
    await tx.outboxEvent.create({
      data: {
        ...
        event_type: 'greedy_classic.round.cancelled', socket_room: GREEDY_CLASSIC_SOCKET_ROOM,
...
const requestPublishConfig = async (config_id: string, context: AdminAuditContext) =>
  prisma.$transaction(async (tx) => {
    ...
    const game = await getGameOrThrow(tx);
    const target = await tx.greedyClassicConfigVersion.findFirst({ where: { id: config_id, game_id: game.id }, include: { options: true, chip_values: true } });
    ...
    const approval = await createPendingApproval({ admin_user_id, action_type: 'greedy_classic.config.publish', target_type: 'greedy_classic_config_version', target_id: target.id, payload: { config_id: target.id, version: target.version }, idempotency_key }, tx);
    await tx.greedyClassicConfigVersion.update({ where: { id: target.id }, data: { status: ConfigVersionStatus.review_pending } });
    await writeAdminAudit(tx, { ...context, approval_request_id: approval.id, outcome: 'success' }, { action: 'greedy_classic.config.submitted_for_approval', entity_type: 'greedy_classic_config_version', entity_id: target.id, new_values: { version: target.version, approval_id: approval.id } });
...
const publishApprovedConfig = async (approval_id: string, context: AdminAuditContext) =>
  prisma.$transaction(async (tx) => {
    ...
    if (!approval || approval.action_type !== 'greedy_classic.config.publish') throw new AppError(httpStatus.NOT_FOUND, 'Config publish approval not found');
    ...
    const game = await getGameOrThrow(tx);
    const target = await tx.greedyClassicConfigVersion.findFirst({ where: { id: payload.config_id, game_id: game.id, status: ConfigVersionStatus.review_pending }, include: { options: true, chip_values: true } });
    if (!target) throw new AppError(httpStatus.CONFLICT, 'Config is no longer awaiting approval');
    const now = new Date();
    await tx.greedyClassicConfigVersion.updateMany({ where: { game_id: game.id, status: ConfigVersionStatus.published }, data: { status: ConfigVersionStatus.retired, retired_at: now } });
    const published = await tx.greedyClassicConfigVersion.update({ where: { id: target.id }, data: { status: ConfigVersionStatus.published, published_at: now }, include: { options: { orderBy: { display_order: 'asc' } }, chip_values: { orderBy: { display_order: 'asc' } } } });
    await tx.greedyClassicRuntimeState.update({ where: { game_id: game.id }, data: { active_config_version_id: target.id, revision: { increment: 1 } } });
    await markApprovalApplied(tx, approval.id, context);
    await writeAdminAudit(tx, { ...context, approval_request_id: approval.id, outcome: 'success' }, { action: 'greedy_classic.config.published', entity_type: 'greedy_classic_config_version', entity_id: target.id, new_values: { version: target.version } });
    return published;
  });
