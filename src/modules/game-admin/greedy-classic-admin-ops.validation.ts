import { z } from 'zod';
import { AdminRole, GreedyClassicRoundStatus } from '@/generated/prisma/client';
import { createGreedyClassicConfigSchema } from './game-admin.validation';
...
export const roundListSchema = z.object({
  query: paginationSchema.extend({
    status: z.nativeEnum(GreedyClassicRoundStatus).optional(),
...
export const configBodySchema = createGreedyClassicConfigSchema;
export const configUpdateSchema = z.object({
  params: z.object({ config_id: z.string().trim().cuid() }),
  body: createGreedyClassicConfigSchema.shape.body,
});
